...
public abstract class AbstractGraphK2 implements Graph {
  ...
  protected AbstractGraphK2(int[][] edges, int numberOfVertices) {
    vertices = new Integer[numberOfVertices]; // Create vertices
    for (int i = 0; i < numberOfVertices; i++) {
      vertices[i] = new Integer(i); // vertices is {0, 1, 2, ...}
    }
...
//getConnectedComponents
  public List<List<Integer>> getConnectedComponents()
  {
	  ...
	  for(int x = 0; x < Bob.size(); x++)
	  {
		  ...
		  //if path does not utilize every vertex
		  if(Bob.get(x).size() != getSize())
		  {
			  //remove incomplete path from list of lists
			  Bob.remove(x);
			  x--;
		  }
