...
public class RatioDriver1 
{
	...
	public static void main(String[] args) 
	{
		...
		//declare tool variables
			//number of vertices
				//this cannot exceed 11 as the total number of graphs will not be able to be held by a long variable
		long n = 8;
		...
		while(flag == true)
		{
			...
			trip = true;
			//walk through the last edges worth of edges in graph for current edge number completion
			for(long y = 1; y <= edges; y++)
			{
				if( graph.get((int) (graph.size()-y)) == false )
				{
					trip = false;
				}
			}
