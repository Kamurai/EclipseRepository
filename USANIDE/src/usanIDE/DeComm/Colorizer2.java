...
public class Colorizer2 extends DefaultHighlighter 
{
	...
	public Colorizer2(JTextPane textpane, ArrayList<ArrayList<String>> input)
	{
	...
	public void ColorItAll3(JTextPane textpane, ArrayList<ArrayList<String>> input)
	{
		...
		int minView = textpane.getCaretPosition() - 500;
		int maxView = textpane.getCaretPosition() + 500;
		...
		if(input.size() > 0)
		{
			...
			{
				//color black  from the beginning of the document (view) to 1 place before the first colored word 
				Start = 0;
				...
				for(int x = 0; x < input.size(); x++)
				{
					...
					else //this is the last word to be colored
					{
						...
						if(Integer.parseInt(input.get(x).get(1)) < textpane.getStyledDocument().getLength()-1)
						{
							...
							Start = Integer.parseInt(input.get(x).get(1))+1;
							Length   = ((textpane.getStyledDocument().getLength()-1)-Start);
