...
public class CallVariableManager2 
{
	ManagerHelper Sandslash;
	
	public CallVariableManager2()
	{
					...
					else if(mode == 3) //not a comment mode
					{
						...
						else
						{
									...
									if((content.charAt(tracker+1) == '_'))
									{
										...
										for(; tracker < content.length(); tracker++)
										{
											...
											if((content.charAt(tracker) == ','))
											{
												...
												else if(Sandslash.CallVariableIsDuplicate(potentialWord, startofVariableIndex,endofVariableIndex, content))
												{
													errorFound = true;
													errorType = 12;
												...
												else if((potentialWord.substring(0,2).compareTo("c_") == 0) && !(Sandslash.IsApprovedCellVariable(potentialWord)) )
												{
													errorFound = true;
													errorType = 13;
													tracker = Sandslash.findEndOfLine(tracker, content);
							...
							if(errorFound)
							{
								...
								{
									priority = 2;
									description = "Variable is not an expected call variable.";
								}
