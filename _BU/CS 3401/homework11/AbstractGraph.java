...
//getConnectedPaths
  public List<List<Integer>> getConnectedPaths()
  {
	  ...
	  //check for paths from every vertex
	  for(int z = 0; z < Tim.length; z++)
	  {
		  //clear visitation list each time
		  for(int y = 0; y < Tim.length; y++)
		  {
			  Tim[y] = 0;
		  }
	  
	  //check the visitation list from the target vertex
	  for(int x = z; x < Tim.length; x++)
	  {
		  flag = true;
  ...
  {
	  boolean cycle = false;
	  List<List<Integer>> Bob = getConnectedPaths();
	  List<Integer> Adam = new LinkedList<Integer>();
