...
public class BinaryTree<E extends Comparable<E>> 
  ...
  public String breadthFirst()
  {
	  ...
	  String result = stringATree("String");
  ...
  public int countLeaves()
  {
	  ...
	  int count = Integer.parseInt(stringATree("Count"));		  
  ...
  public String stringATree(String condition)
  {
	  
	  if(root == null)
	  {
		  if(condition.equals("Count"))
		  {
			return "0";  
		  }
	  ...
	  //could use LinkedList of TreeNodes to process breadth
		  	ArrayList<TreeNode<E>> monkeys = new ArrayList<TreeNode<E>>();
		  	//add one when needed
		  	monkeys.add(root);
	
		  	for(int x = 0; x < monkeys.size(); x++)
		  	{
		  		if(monkeys.get(x).left != null)
		  		{
		  			monkeys.add(monkeys.get(x).left);
		  		}
		  		if(monkeys.get(x).right != null)
		  		{
		  			monkeys.add(monkeys.get(x).right);
		  		}
		  		
		  		if(monkeys.get(x).left == null && monkeys.get(x).right == null)
		  		{
		  			count++;
		  		}
		  		
		  		result += (monkeys.get(x).element).toString();
